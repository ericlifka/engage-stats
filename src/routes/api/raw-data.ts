export const UNITS_RAW = [
    ["Alear", 1, "Dragon Child", 22, 6, 0, 5, 7, 5, 3, 5, 4],
    ["Vander", 1, "Paladin", 40, 11, 5, 10, 8, 10, 8, 6, 8],
    ["Clanne", 1, "Mage", 19, 1, 8, 11, 9, 4, 7, 4, 4],
    ["Framme", 1, "Martial Monk", 18, 3, 5, 8, 7, 4, 8, 5, 3],
    ["Alfred", 5, "Noble (Alfred)", 27, 9, 2, 10, 6, 8, 5, 8, 6],
    ["Etie", 4, "Archer", 21, 10, 0, 11, 6, 3, 2, 8, 4],
    ["Boucheron", 4, "Axe Fighter", 29, 10, 0, 8, 7, 6, 3, 5, 9],
    ["Céline", 5, "Noble (Céline)", 22, 7, 9, 9, 10, 5, 7, 11, 4],
    ["Chloé", 6, "Lance Flier", 26, 8, 4, 13, 13, 5, 9, 10, 5],
    ["Louis", 6, "Lance Armor", 30, 13, 0, 11, 3, 16, 1, 4, 8],
    ["Yunaka", 6, "Thief", 25, 8, 5, 14, 12, 9, 7, 8, 5],
    ["Alcryst", 10, "Lord (Alcryst)", 28, 11, 2, 17, 12, 8, 5, 8, 6],
    ["Citrinne", 10, "Mage", 24, 2, 15, 13, 10, 3, 12, 10, 4],
    ["Lapis", 10, "Sword Fighter", 26, 11, 2, 12, 14, 7, 6, 7, 5],
    ["Diamant", 11, "Lord (Diamant)", 32, 13, 3, 13, 13, 10, 6, 10, 9],
    ["Amber", 11, "Lance Cavalier", 31, 15, 2, 13, 11, 9, 3, 9, 8],
    ["Jade", 12, "Axe Armor", 33, 14, 4, 14, 5, 18, 6, 5, 8],
    ["Ivy", 17, "Wing Tamer (Ivy)", 32, 7, 17, 15, 13, 12, 15, 4, 7],
    ["Kagetsu", 1, "Swordmaster", 35, 17, 5, 22, 22, 14, 10, 17, 9],
    ["Zelkov", 17, "Thief", 35, 15, 3, 19, 19, 14, 5, 7, 9],
    ["Fogado", 17, "Sentinel (Fogado)", 33, 12, 7, 17, 18, 10, 11, 8, 8],
    ["Pandreo", 1, "High Priest", 34, 7, 16, 18, 17, 8, 22, 14, 8],
    ["Bunet", 1, "Great Knight", 41, 15, 5, 15, 9, 20, 8, 13, 11],
    ["Timerra", 18, "Sentinel (Timerra)", 35, 14, 6, 17, 18, 16, 8, 10, 5],
    ["Panette", 1, "Berserker", 46, 25, 3, 19, 13, 11, 7, 11, 11],
    ["Merrin", 1, "Wolf Knight", 36, 15, 10, 21, 21, 12, 12, 14, 9],
    ["Hortensia", 19, "Wing Tamer (Hort.)", 27, 5, 12, 21, 19, 6, 23, 17, 5],
    ["Seadall", 15, "Dancer", 33, 12, 4, 15, 16, 10, 10, 11, 8],
    ["Rosado", 3, "Wyvern Knight", 44, 16, 7, 19, 21, 15, 11, 8, 8],
    ["Goldmary", 3, "Hero", 40, 17, 2, 18, 20, 20, 12, 13, 9],
    ["Lindon", 5, "Sage", 38, 8, 19, 21, 19, 12, 21, 8, 10],
    ["Saphir", 7, "Warrior", 51, 24, 2, 23, 19, 19, 9, 15, 15],
    ["Veyle", 35, "Fell Child", 35, 20, 26, 24, 23, 17, 29, 18, 7],
    ["Mauvier", 12, "Royal Knight", 51, 21, 23, 25, 22, 26, 24, 16, 12],
    ["Anna", 5, "Axe Fighter", 29, 10, 2, 9, 7, 4, 5, 3, 7],
    ["Jean", 1, "Martial Monk", 19, 4, 5, 10, 7, 4, 8, 3, 3],
];

export const UNITS_GROWTHS_RAW = {
    "Alear": [60, 35, 20, 45, 50, 40, 25, 25, 5],
    "Vander": [60, 25, 10, 25, 35, 35, 20, 10, 5],
    "Clanne": [40, 35, 10, 40, 50, 30, 25, 20, 5],
    "Framme": [55, 30, 25, 35, 55, 25, 30, 25, 0],
    "Alfred": [65, 40, 5, 35, 40, 40, 20, 40, 10],
    "Etie": [45, 40, 0, 25, 35, 25, 30, 25, 5],
    "Boucheron": [85, 20, 0, 50, 45, 35, 20, 15, 20],
    "Céline": [50, 35, 25, 30, 45, 30, 40, 50, 5],
    "Chloé": [75, 25, 35, 40, 55, 30, 25, 25, 5],
    "Louis": [75, 40, 0, 25, 25, 50, 20, 25, 15],
    "Yunaka": [50, 35, 25, 40, 45, 15, 45, 25, 5],
    "Alcryst": [65, 30, 10, 40, 45, 30, 20, 15, 10],
    "Citrinne": [45, 10, 40, 25, 30, 20, 40, 25, 5],
    "Lapis": [55, 25, 20, 35, 55, 35, 30, 25, 5],
    "Diamant": [75, 30, 15, 20, 40, 40, 25, 20, 15],
    "Amber": [65, 45, 0, 25, 30, 35, 5, 35, 15],
    "Jade": [55, 35, 25, 35, 30, 40, 30, 20, 10],
    "Ivy": [55, 25, 30, 25, 40, 30, 35, 15, 10],
    "Kagetsu": [60, 30, 15, 50, 50, 40, 25, 40, 10],
    "Zelkov": [65, 35, 15, 40, 35, 35, 15, 25, 10],
    "Fogado": [60, 30, 25, 30, 55, 30, 35, 25, 10],
    "Pandreo": [60, 5, 30, 45, 45, 15, 55, 30, 15],
    "Bunet": [65, 30, 10, 40, 35, 45, 25, 40, 10],
    "Timerra": [55, 25, 25, 45, 45, 30, 30, 30, 10],
    "Panette": [75, 45, 10, 40, 25, 30, 15, 20, 15],
    "Merrin": [55, 25, 25, 40, 50, 30, 30, 25, 10],
    "Hortensia": [40, 20, 20, 35, 50, 25, 55, 50, 0],
    "Seadall": [55, 25, 15, 25, 50, 25, 25, 35, 10],
    "Rosado": [75, 45, 25, 40, 45, 30, 30, 20, 5],
    "Goldmary": [65, 30, 5, 25, 25, 55, 25, 25, 5],
    "Lindon": [65, 25, 25, 25, 40, 25, 40, 15, 10],
    "Saphir": [80, 35, 0, 25, 30, 30, 5, 20, 10],
    "Veyle": [40, 25, 45, 35, 30, 25, 35, 20, 0],
    "Mauvier": [70, 35, 40, 40, 35, 50, 45, 15, 15],
    "Anna": [55, 15, 50, 50, 50, 20, 35, 45, 5],
    "Jean": [50, 20, 20, 35, 40, 25, 20, 25, 5],
}

export const CLASS_STATS_RAW = {
    "Dragon Child": [22, 6, 0, 2, 7, 5, 3, 2, 4],
    "Divine Dragon": [24, 8, 1, 4, 8, 6, 5, 3, 7],
    "Fell Child": [20, 3, 7, 4, 5, 2, 7, 3, 4],
    "Noble (Alfred)": [24, 7, 2, 6, 5, 6, 4, 4, 6],
    "Avenir": [25, 8, 4, 7, 6, 8, 6, 6, 7],
    "Noble (Céline)": [20, 5, 8, 5, 8, 3, 5, 4, 4],
    "Vidame": [21, 6, 9, 6, 9, 4, 6, 5, 5,],
    "Lord (Diamant)": [23, 8, 1, 7, 8, 5, 3, 3, 7],
    "Successeur": [25, 10, 2, 8, 9, 6, 3, 4, 8],
    "Lord (Alcryst)": [21, 7, 1, 10, 7, 4, 3, 2, 5],
    "Tireur d’élite": [22, 8, 1, 12, 8, 6, 4, 4, 6],
    "Wing Tamer (Ivy)": [20, 1, 8, 4, 4, 6, 8, 1, 5],
    "Lindwurm": [22, 2, 10, 5, 5, 8, 9, 1, 6],
    "Wing Tamer (Hort.)": [18, 0, 5, 9, 8, 1, 11, 6, 4],
    "Sleipnir Rider": [18, 1, 7, 10, 10, 2, 13, 8, 5],
    "Sentinel (Timerra)": [24, 8, 1, 5, 7, 8, 1, 3, 4],
    "Picket": [25, 9, 2, 6, 9, 10, 3, 4, 5],
    "Sentinel (Fogado)": [21, 5, 1, 9, 7, 4, 3, 2, 6],
    "Cupido": [23, 6, 3, 10, 8, 5, 5, 3, 7],
    "Melusine": [22, 7, 8, 6, 8, 6, 9, 2, 6],
    "Sword Fighter": [20, 5, 0, 7, 8, 3, 2, 2, 5],
    "Swordmaster": [21, 6, 1, 9, 11, 4, 3, 4, 6],
    "Hero": [23, 8, 0, 8, 9, 5, 2, 3, 7],
    "Lance Fighter": [23, 7, 2, 8, 6, 4, 2, 2, 5],
    "Halberdier": [24, 9, 1, 9, 7, 6, 2, 3, 6],
    "Royal Knight": [23, 7, 5, 9, 8, 5, 4, 5, 6],
    "Axe Fighter": [26, 9, 0, 5, 5, 3, 1, 1, 7],
    "Berserker": [29, 13, 0, 6, 6, 3, 2, 2, 9],
    "Warrior": [27, 12, 1, 7, 7, 4, 3, 2, 8],
    "Archer": [19, 6, 0, 9, 5, 2, 1, 2, 4],
    "Sniper": [22, 8, 1, 11, 6, 3, 1, 3, 5],
    "Bow Knight": [22, 7, 2, 10, 8, 3, 3, 3, 5],
    "Sword Armor": [25, 8, 0, 6, 1, 12, 0, 2, 7],
    "Lance Armor": [25, 8, 0, 6, 1, 12, 0, 2, 7],
    "Axe Armor": [25, 8, 0, 6, 1, 12, 0, 2, 7],
    "General": [28, 11, 1, 7, 2, 14, 1, 3, 10],
    "Great Knight": [26, 9, 2, 8, 5, 13, 2, 3, 8],
    "Sword Cavalier": [23, 6, 1, 8, 7, 4, 2, 2, 6],
    "Lance Cavalier": [23, 6, 1, 8, 7, 4, 2, 2, 6],
    "Axe Cavalier": [23, 6, 1, 8, 7, 4, 2, 2, 6],
    "Paladin": [25, 8, 2, 10, 8, 6, 3, 3, 7],
    "Wolf Knight": [23, 6, 3, 9, 10, 4, 4, 4, 6],
    "Mage": [18, 1, 7, 6, 6, 1, 7, 2, 4],
    "Sage": [20, 1, 9, 8, 7, 3, 9, 3, 5],
    "Mage Knight": [21, 5, 7, 8, 9, 3, 8, 2, 6],
    "Martial Monk": [18, 3, 5, 6, 5, 3, 8, 3, 3],
    "Martial Master": [22, 6, 5, 6, 5, 4, 7, 4, 6],
    "High Priest": [20, 3, 8, 8, 6, 3, 10, 5, 4],
    "Sword Flier": [21, 5, 2, 7, 9, 3, 7, 3, 4],
    "Lance Flier": [21, 5, 2, 7, 9, 3, 7, 3, 4],
    "Axe Flier": [21, 5, 2, 7, 9, 3, 7, 3, 4],
    "Griffin Knight": [22, 7, 3, 10, 11, 4, 9, 5, 5],
    "Wyvern Knight": [25, 9, 1, 8, 9, 6, 5, 3, 6],
    "Thief": [22, 5, 0, 10, 10, 6, 2, 2, 4],
    "Dancer": [21, 5, 1, 8, 8, 2, 5, 5, 5],
    "Enchanter": [20, 5, 5, 5, 5, 5, 5, 5, 5],
    "Mage Cannoneer": [20, 5, 5, 5, 5, 5, 5, 5, 5],
};

export const CLASS_GROWTHS_RAW = {
    "Dragon Child": [10, 10, 0, 10, 15, 10, 10, 5, 5],
    "Divine Dragon": [10, 10, 0, 10, 15, 15, 15, 10, 5],
    "Fell Child": [10, 10, 25, 10, 5, 10, 25, 0, 0],
    "Noble (Alfred)": [15, 15, 0, 5, 5, 20, 5, 15, 0],
    "Avenir": [15, 15, 0, 5, 10, 25, 5, 20, 5],
    "Noble (Céline)": [0, 5, 10, 5, 5, 5, 10, 20, 0],
    "Vidame": [5, 10, 15, 5, 5, 5, 15, 20, 0],
    "Lord (Diamant)": [15, 20, 0, 10, 10, 10, 0, 5, 0],
    "Successeur": [15, 20, 0, 10, 15, 15, 0, 10, 5],
    "Lord (Alcryst)": [10, 10, 0, 25, 10, 10, 0, 5, 0],
    "Tireur d’élite": [10, 10, 0, 30, 10, 15, 5, 5, 0],
    "Wing Tamer (Ivy)": [5, 0, 20, 0, 0, 10, 20, 0, 0],
    "Lindwurm": [5, 0, 25, 5, 0, 15, 25, 0, 0],
    "Wing Tamer (Hort.)": [0, 0, 0, 15, 10, 10, 0, 25, 10],
    "Sleipnir Rider": [0, 0, 20, 15, 15, 0, 30, 15, 0],
    "Sentinel (Timerra)": [10, 10, 0, 10, 5, 20, 0, 5, 0],
    "Picket": [10, 15, 0, 10, 10, 20, 5, 5, 5],
    "Sentinel (Fogado)": [10, 5, 0, 15, 15, 5, 15, 0, 0],
    "Cupido": [10, 5, 5, 20, 20, 5, 20, 0, 0],
    "Melusine": [10, 10, 10, 10, 15, 10, 10, 0, 5],
    "Sword Fighter": [10, 10, 0, 15, 20, 0, 15, 10, 0],
    "Swordmaster": [10, 10, 0, 15, 20, 0, 15, 15, 0],
    "Hero": [15, 15, 0, 10, 15, 0, 10, 15, 0],
    "Lance Fighter": [10, 10, 5, 20, 5, 10, 5, 0, 0],
    "Halberdier": [10, 15, 5, 20, 10, 15, 5, 5, 0],
    "Royal Knight": [5, 10, 15, 15, 15, 5, 20, 10, 0],
    "Axe Fighter": [25, 20, 0, 5, 10, 5, 0, 0, 5],
    "Berserker": [30, 30, 0, 5, 10, 5, 0, 0, 10],
    "Warrior": [25, 20, 0, 10, 15, 10, 5, 0, 5],
    "Archer": [10, 15, 0, 25, 10, 5, 0, 5, 0],
    "Sniper": [15, 15, 0, 30, 10, 10, 0, 10, 0],
    "Bow Knight": [10, 10, 0, 20, 20, 0, 15, 15, 0],
    "Sword Armor": [20, 15, 0, 10, 0, 30, 0, 0, 5],
    "Lance Armor": [20, 15, 0, 10, 0, 30, 0, 0, 5],
    "Axe Armor": [20, 15, 0, 10, 0, 30, 0, 0, 5],
    "General": [25, 20, 0, 10, 0, 30, 0, 0, 10],
    "Great Knight": [20, 15, 0, 15, 0, 25, 10, 5, 5],
    "Sword Cavalier": [10, 10, 0, 10, 10, 10, 10, 10, 0],
    "Lance Cavalier": [10, 10, 0, 10, 10, 10, 10, 10, 0],
    "Axe Cavalier": [10, 10, 0, 10, 10, 10, 10, 10, 0],
    "Paladin": [15, 15, 0, 10, 15, 15, 15, 10, 0],
    "Wolf Knight": [10, 5, 0, 15, 20, 5, 20, 20, 0],
    "Mage": [0, 0, 25, 5, 0, 0, 25, 5, 0],
    "Sage": [0, 0, 30, 5, 0, 0, 30, 15, 0],
    "Mage Knight": [5, 5, 25, 0, 5, 10, 25, 5, 0],
    "Martial Monk": [0, 10, 25, 0, 0, 10, 20, 10, 0],
    "Martial Master": [5, 10, 20, 0, 0, 15, 25, 10, 0],
    "High Priest": [0, 0, 25, 0, 5, 0, 30, 30, 0],
    "Sword Flier": [5, 10, 5, 10, 10, 0, 10, 10, 0],
    "Lance Flier": [5, 10, 5, 10, 10, 0, 10, 10, 0],
    "Axe Flier": [5, 10, 5, 10, 10, 0, 10, 10, 0],
    "Griffin Knight": [0, 10, 15, 15, 20, 0, 15, 15, 0],
    "Wyvern Knight": [20, 20, 0, 10, 5, 20, 5, 5, 5],
    "Thief": [5, 10, 0, 20, 15, 15, 5, 15, 0],
    "Dancer": [5, 10, 0, 5, 15, 10, 20, 20, 0],
    "Enchanter": [5, 15, 15, 15, 10, 5, 5, 15, 5],
    "Mage Cannoneer": [15, 10, 0, 15, 5, 20, 5, 10, 10],
}